...
public sealed class GetMetadataTask : IDisposable
{
    private GodotObject taskObject;

    public GetMetadataTask(GodotObject godotObject) => Set(godotObject);
    public GetMetadataTask(Action<Metadata> onCompleted, Action<Storage.Error, string> onError)
    {
        if (onCompleted != null) Completed += onCompleted;
        if (onError != null) Error += onError;
    }
    public GetMetadataTask Set(GodotObject godotObject)
    {
