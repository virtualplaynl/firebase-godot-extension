...
public sealed class UpdateMetadataTask : IDisposable
{
    private GodotObject taskObject;

    public UpdateMetadataTask(GodotObject godotObject) => Set(godotObject);
    public UpdateMetadataTask(Action<Metadata> onCompleted, Action<Storage.Error, string> onError)
    {
        if (onCompleted != null) Completed += onCompleted;
        if (onError != null) Error += onError;
    }
    public UpdateMetadataTask Set(GodotObject godotObject)
    {
