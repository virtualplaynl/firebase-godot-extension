...
public sealed class PutFileTask : IDisposable
{
    private GodotObject taskObject;

    public PutFileTask(GodotObject godotObject) => Set(godotObject);
    public PutFileTask(Action<Metadata> onCompleted, Action<Storage.Error, string> onError)
    {
        if (onCompleted != null) Completed += onCompleted;
        if (onError != null) Error += onError;
    }
    public PutFileTask Set(GodotObject godotObject)
    {
